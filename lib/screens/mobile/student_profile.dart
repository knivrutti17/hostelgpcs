                      ...
                      child: Column(
                        children: [
                          GridView.count(
                            ...
                            children: [
                              _infoCard(Icons.assignment_outlined,
                                  "Roll Number", data['rollNo'] ?? '---'),
                              _infoCard(Icons.business_center_outlined,
                                  "Branch", data['department'] ?? 'IT'),
                              _infoCard(Icons.category_outlined, "Category",
                                  data['category'] ?? 'Open'),
                              _infoCard(Icons.home_outlined, "Hostel",
                                  data['hostel'] ?? 'Devgiri'),
                              _infoCard(Icons.phone_outlined, "Contact",
                                  data['contact'] ?? '---'),
                              _infoCard(
                                  Icons.meeting_room_outlined,
                                  "Room Number",
                                  data['roomNo']?.toString() ?? '---'),
                              _infoCard(Icons.bloodtype_outlined, "Blood Group",
                                  data['bloodGroup'] ?? '---'),
                              _infoCard(Icons.hotel_outlined, "Hostel",
                                  data['hostel'] ?? 'Devgiri'),
                            ],
                          ),
                          ...
                          _actionRow(Icons.edit_outlined, "Edit Profile", () {
                            Navigator.push(
                                ...
                                MaterialPageRoute(
                                    builder: (context) =>
                                        ProfileEdit(studentData: data)));
                          }),
                          const SizedBox(height: 12),
                          _actionRow(Icons.badge_outlined, "Download ID Card",
                              () async {
                            ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(
                                    content: Text("Generating ID Card..."),
                                    duration: Duration(seconds: 2)));
                            await IDCardGenerator.generateAndDownloadIDCard(
                                data);
                          }),
                          const SizedBox(height: 12),
    ...
    return Container(
      ...
      decoration: const BoxDecoration(
        gradient: LinearGradient(
            colors: [Color(0xFFA8E6CF), Color(0xFFF8F9FE)],
          ...
          Container(
            ...
            child: CircleAvatar(
              ...
              child: imageBytes == null
                  ? const Icon(Icons.person, size: 70, color: Color(0xFFBCC6D1))
                  : null,
            ),
          ),
      ...
      children: [
        _quickActionBtn(Icons.edit_note, "Edit Profile", () {
          Navigator.push(
              context,
              MaterialPageRoute(
                  builder: (context) => ProfileEdit(studentData: data)));
        }),
        _quickActionBtn(Icons.lock_reset, "Change PW", () {}),
        _quickActionBtn(Icons.badge_outlined, "ID Card",
            () async => await IDCardGenerator.generateAndDownloadIDCard(data)),
      ],
    );
  }
      ...
      onTap: () async {
        ...
        if (context.mounted) {
          Navigator.pushNamedAndRemoveUntil(
              context, '/mobile_login', (route) => false);
        }
      },
      child: Container(
